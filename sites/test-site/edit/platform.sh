...
full_deploy () {
  ...
  rm -rf ${ROOT_DIR}
  if [[ ${PLATFORM_BRANCH} =~ ^pr- ]]; then
    ${CMD_GIT} -c credential.helper="!f() { echo username=wronguser; echo password=wrongpassword; }; f" clone ${APP_GIT_REMOTE_URL} ${ROOT_DIR}
    cd ${ROOT_DIR}
    ID=$(echo $PLATFORM_BRANCH | sed s/pr-//g)
    git fetch origin pull/${ID}/head:${PLATFORM_BRANCH}
    git checkout ${PLATFORM_BRANCH}
  else
    ${CMD_GIT} -c credential.helper="!f() { echo username=wrongpassword; echo password=wrongpassword; }; f" clone -b ${PLATFORM_BRANCH} ${APP_GIT_REMOTE_URL} ${ROOT_DIR}
    cd ${ROOT_DIR}
  fi
